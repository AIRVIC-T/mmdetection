...
from mmdet.structures.bbox import BaseBoxes, cat_boxes, get_box_tensor, scale_boxes, get_box_wh
...
class QualityRPNHead(AnchorFreeHead):
    def __init__(self,
                 in_channels: int,
                 num_classes: int = 1,
                 start_level: int = 0,
    ...
    def _init_layers(self) -> None:
        ...
        if self.enable_dam:
            ...
            self.reg_decomp = DecoupledAttentionModule(self.feat_channels,
                                                       ...
                                                       self.conv_cfg, self.norm_cfg)
        self.relu = nn.ReLU()
        self.qpn_cls = nn.Conv2d(
            self.feat_channels,
            self.num_base_priors * self.cls_out_channels,
            3,
            padding=1)
        self.qpn_reg = nn.Conv2d(
    ...
    def init_weights(self) -> None:
        ...
        normal_init(self.qpn_cls, std=0.01, bias=bias_cls)
        normal_init(self.qpn_reg, std=0.01)
    ...
    def forward_single(self, x: Tensor, scale: Scale, stride: int) -> Sequence[Tensor]:
        ...
        cls_score = self.qpn_cls(cls_feat)
        bbox_pred = scale(self.qpn_reg(reg_feat).exp()).float() * stride
    ...
    def loss_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        if len(pos_inds) > 0:
            ...
            pos_prior_points = self.anchor_center(pos_anchors)
    ...
    def _bbox_post_process(self,
        ...
        if results.bboxes.numel() > 0:
            ...
            _, keep_idxs = batched_nms(bboxes, results.scores,
            ...
            if isinstance(results.bboxes, BaseBoxes):
                ...
                results_.bboxes = results.scores.new_zeros(0, 5)
